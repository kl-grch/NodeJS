...
/* Найстройка десериализации, передаётся логин, а из базы по этому логину находим объект пользователя */
passport.deserializeUser( (login, cb)=> {
db.findUser(login, cb);
});
/*
Единственная экспортируемая функция из модуля, запускает создание локальной стратегией проверки пользователя с кодом проверки
*/
function initPassport () {
/* Задаём модулю паспорт по какой стратегии он будет работать и создаём новую локальную стратегию передавая опции и функцию, которая определяет три состояния: ошибка, пользователь не найден с таким логином и паролем и пользователь есть доступ разрешён */
passport.use(new LocalStrategy(options,
/* username – то что ввёл пользователь в качестве логина, password – то что пользователь ввёл в качестве пароля, функция обратного вызова */
(username, password, done) => {
/*Запрашиваем в хранилище пользователей конкретного пользователя по присланному логину, в функцию обратного вызова вернется ошибка или объект описывающий пользователя (переменная user) */
...
.digest('hex');
/*Проверяем на соответствие шифрованного пароля, тому что лежит в базе, и возвращаем в passport сигнал что доступа нет, так как пароль не тот, или весь объект описывающий пользователя как сигнал что всё норм */
