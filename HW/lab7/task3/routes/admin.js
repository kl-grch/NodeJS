...
/* Подключаем промежуточный обработчик, который проверяет авторизован ли пользователь, если авторизован передает управление в другой обработчик, если нет то перенаправляем на путь /admin */
...
/*Роут организующий прием данных со странички аутентификации. Полный путь: /admin/. Внутри осуществляется проверка логина и пароля c помощью passport. Если проверка пройдена, в функцию
обратного вызова придёт объект пользователя (user) и перенаправляем на страницу /admin. Если проверка не пройдена возвращаем страницу аутентификации с сообщением об ошибке */
...
/*Роут организующий прием запрос о выходе текущего пользователя из системы. Полный путь: /admin/out. Внутри осуществляется посыл сигналу модулю passport, что пользователь выходит из системы и перенаправление пользователя на /admin */
route.post('/out', (req, res, next)=>{
req.logout();
res.redirect('/admin');
});
/*Роут организующий отдачу секретной информации. Вторым аргументов передается промежуточный обработчик, который проверяет авторизован ли пользователь или нет. Полный путь: /admin/secret */
